<div class="footer">
    <div class="block">
        
        <div class="pure-g">
            <div class="pure-u-1-3">
            
       <div class="footer-info">  
        
<div class="footer_logo">
    <span><a href="/">ИПРИМ РАН</a></span>
</div>

<div class="small_history">
  
   Ведущий институт работающий по направлениям механики с использованием достижений смежных наук – физики, математики, химии.   </div>  
            
<div class="footer_adress">
<ul>
    <li><i class="icon-metro-two"></i> Белорусская</li>
    <li>125040, Россия, Москва, Ленинградский проспект д.7</li>
    <li>+7 495 946-18-06</li>
    <li><a href="mailto:iam@iam.ras.ru">iam@iam.ras.ru</a></li>
</ul>
</div> 
  
   </div>              
            </div>
            
            <div class="pure-u-2-3">
            
 <div class="pure-g">
            
            <div class="pure-u-1-2">
     
<div class="footer_head">Разделы</div>                 
<ul class="">
   <li><a href="/history" >Об Институте</a></li>
   <li><a href="/education/">Образовательная деятельность</a></li>
   <li><a href="http://iam.ras.ru/employees/">Сотрудники</a></li>
   <li><a href="/conference/" >Конференции</a></li>
   <li><a href="/srw/">Договора</a></li>
</ul>                
                
            </div>
            
            <div class="pure-u-1-3">
            
<div class="footer_head">Еще</div>          
 <ul>
    <li><a href="/getmail/">Заявка на почту</a></li>
    <li><a href="https://mail.yandex.ru/?pdd_domain=iam.ras.ru">Вход в почту</a></li>
    
    <li><a href="/mkmk/">Журнал МКМК</a></li>
    <li><a href="/continuum_mechanics/">Механика сплошных сред</a></li>
    
    <li><a href="https://www.youtube.com/channel/UC5sZXxnq-14Fx8Muyj3NK6A"><i class="icon-youtube"></i></a> <a href="https://plus.google.com/103876993466338875009/videos"><i class="icon-google-plus-1"></i></a> <a href="https://github.com/KsiderCorp/"><i class="icon-hierarchy-2"></i></a></li>
    
</ul>               
                
            </div>
            
            <div class="pure-u-1-6 right">
               
<a href="/webinfo">
<h2 class="design"><i class="icon-43-astronaut-and-flag"></i></h2>
</a> 
<?php if( current_user_can('administrator') ){ echo get_num_queries(); ?> | <?php timer_stop(1); } ?>    
                
            </div> 
               
</div>                                    
                
            </div>
            

            
            
        </div>
        
        
        <div class="footer-line">
        <div class="pure-g">
            <div class="pure-u-1-2">
<div class="copyright">
1985<script>new Date().getFullYear()>2010&&document.write("-"+new Date().getFullYear());</script>, <span>Институт прикладной механики Российской академии наук</span> 
</div>
           </div>
           
            <div class="pure-u-1-3">
По вопросам работы сайта можно писать на <a href="mailto:info@iam.ras.ru">info@iam.ras.ru</a>                 
            </div>
            
            <div class="pure-u-1-6 right">
                 <div class="design_logo">
                     <a href="http://nikolaysemenov.ru">Разработка</a>
                 </div> 
 
            </div>
        </div>

        

       
        </div>
        
    </div>
</div>